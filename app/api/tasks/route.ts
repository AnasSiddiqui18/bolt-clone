...
async function processTaskWithTimeout(
    ...
    const warningTimeout = setTimeout(
        async () => {
            try {
                ...
                await logger.info(
                    'Task is taking longer than expected (4+ minutes). Will timeout in 1 minute.',
                )
            } catch (error) {
    ...
    try {
        await Promise.race([processTask(taskId, taskData), timeoutPromise])
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        if (statusesToDelete.length === 0) {
            return NextResponse.json(
                { error: 'No valid actions specified' },
                { status: 400 },
            )
        }

        // Delete tasks based on statuses
        const deletedCount = await deleteTasksByStatus(
            supabase,
            statusesToDelete,
        )
