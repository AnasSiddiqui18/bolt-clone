...
const PricingModal = dynamic(
    () =>
        import('@/components/pricing').then((mod) => ({
...
const Preview = dynamic(
    () =>
        import('@/components/preview').then((mod) => ({
            default: mod.Preview,
        })),
    {
        ssr: false,
...
export default function Home() {
    ...
    const setAuthDialogCallback = useCallback(
        (isOpen: boolean) => {
        ...
        try {
            ...
            if (result?.sbxId) {
                ...
                const response = await fetch(
                    `/api/sandbox/${result.sbxId}/files/content`,
                    {
                        method: 'POST',
                        headers: {
    ...
    return (
        <main className="flex min-h-screen max-h-screen">
            ...
            >
                <div
                    className={`flex flex-col w-full h-screen max-w-[800px] mx-auto px-4 ${
                        fragment || isPreviewPanelOpen
                            ? 'col-span-1'
                            : 'col-span-2'
                    }`}
                >
                    ...
                    <div className="space-y-4 mt-4 ">
                        {error && (
                            <div className="flex items-center justify-between p-2 bg-red-500/10 border border-red-500/20 rounded-lg text-red-500 text-sm">
                                ...
                                <button
                                    onClick={retry}
                                    className="ml-4 p-1 rounded-md hover:bg-red-500/20"
                                >
                                    Retry
                                </button>
                            </div>
