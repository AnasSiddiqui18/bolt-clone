...
export default function AccountSettings() {
    ...
    const loadUserData = useCallback(async () => {
        ...
        setIsLoading(true)
        try {
            const [profile, preferences, securitySettings] =
            ...
            setFormData((prev) => ({
                ...
                emailNotifications:
                    preferences.status === 'fulfilled'
                        ? (preferences.value?.email_notifications ?? true)
                        : true,
                securityAlerts:
                    preferences.status === 'fulfilled'
                        ? (preferences.value?.security_alerts ?? true)
                        : true,
                twoFactorEnabled:
                    securitySettings.status === 'fulfilled'
            ...
            toast({
                title: 'Warning',
                description:
                    "Some account data couldn't be loaded. Basic functionality remains available.",
                variant: 'default',
            })
        } finally {
            setIsLoading(false)
    ...
    const handleUpdateEmail = async () => {
        ...
        try {
            ...
            setFormErrors({
                general: error.message || 'Failed to update email',
            })
            toast({
                title: 'Error',
                description:
    ...
    const handleUpdateNotificationSettings = async (
        key: 'emailNotifications' | 'securityAlerts',
        value: boolean,
    ) => {
    ...
    const handleAvatarUpload = async (
        ...
        if (!file || !session?.user?.id || !supabase || isUploadingAvatar)
            return
                        ...
                        <Button
                            ...
                            onClick={() => {
                                toast({
                                    ...
                                    description:
                                        'Account deletion is not yet implemented. Please contact support if needed.',
                                    variant: 'default',
                                })
                            }}
                        >
