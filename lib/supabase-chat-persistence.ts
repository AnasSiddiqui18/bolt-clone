...
export class SupabaseChatPersistence {
    ...
    static async createSession(
        ...
        const session: ChatSession = {
            ...
            title: initialMessage
                ? ChatUtils.generateSessionTitle(initialMessage.content)
                : 'New Chat',
    ...
    static async exportUserData(
        ...
        for (const session of sessions) {
            try {
                messages[session.sessionId] = await this.getSessionMessages(
                    userId,
                    session.sessionId,
                )
            } catch (error) {
                console.error(
                    `Failed to export messages for session ${session.sessionId}:`,
                    error,
                )
