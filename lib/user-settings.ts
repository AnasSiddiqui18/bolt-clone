...
export async function tableExists(tableName: string): Promise<boolean> {
    ...
    if (
        tablesExistCache[tableName] &&
        now - cacheTime[tableName] < CACHE_DURATION
    ) {
    ...
    try {
        ...
        if (!exists) {
            console.warn(
                `Table ${tableName} does not exist. API calls will return defaults.`,
            )
        }
...
async function safeOperation<T>(
    ...
    if (!(await tableExists(tableName))) {
        console.warn(
            `Skipping ${operationName} - table ${tableName} does not exist`,
        )
        return fallback
    }
            ...
            const { data, error } = await getSupabase()!
                ...
                .insert({
                    ...
                    data_sharing_enabled:
                        preferences?.data_sharing_enabled ?? false,
                    telemetry_enabled: preferences?.telemetry_enabled ?? false,
                    auto_template:
                        preferences?.auto_template ?? 'nextjs-developer',
                    ...preferences,
                })
                .select()
                .single()
...
export async function createUserSecuritySettings(
    userId: string,
): Promise<UserSecuritySettings | null> {
    if (!userId) return null
...
export async function getUserData(userId: string) {
    ...
    const [profile, preferences, integrations, securitySettings] =
        await Promise.all([
