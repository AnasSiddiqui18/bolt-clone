...
export function ChatAnalytics({ className }: ChatAnalyticsProps) {
    ...
    const cleanupOldSessions = async (daysOld: number) => {
        try {
            const response = await fetch(
                `/api/chat/analytics?daysOld=${daysOld}`,
                {
                    method: 'DELETE',
                },
            )
    ...
    const modelData = summary.favoriteModels
        .slice(0, 5)
        .map((model, index) => ({
            name: model,
    ...
    const templateData = summary.favoriteTemplates
        .slice(0, 5)
        .map((template, index) => ({
            name: template,
        ...
        <div className={`space-y-6 ${className}`}>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Manage your chat history and storage
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="space-y-4">
                        {summary.totalTokensUsed && (
                            <div>
                                ...
                                <Progress
                                    value={Math.min(
                                        (summary.totalTokensUsed / 100000) *
                                            100,
                                        100,
                                    )}
                                />
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                            ...
                            <Button
                                variant="outline"
                                onClick={() =>
                                    window.open('/api/chat/export?format=json')
                                }
                                className="flex-1"
                            >
                        ...
                        <p className="text-xs text-muted-foreground">
                            Cleanup removes archived sessions older than
                            specified days. Active sessions are never deleted.
                        </p>
                    </div>
                </CardContent>
            </Card>
