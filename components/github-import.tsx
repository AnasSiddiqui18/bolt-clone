    ...
    const loadRepositories = useCallback(async () => {
        ...
        try {
            ...
            const reposResponse = await fetch(
                `/api/github/repos?owner=${userData.login}`,
            )
            if (!reposResponse.ok) {
                ...
                for (const org of orgs) {
                    try {
                        const orgReposResponse = await fetch(
                            `/api/github/repos?owner=${org.login}`,
                        )
                        ...
                        console.warn(
                            `Failed to fetch repos for org ${org.login}:`,
                            error,
                        )
                    }
                }
            ...
            toast({
                title: 'Error',
                description:
                    'Failed to load GitHub repositories. Please check your connection.',
                variant: 'destructive',
            })
        } finally {
            setIsLoading(false)
    ...
    const importRepository = async (repo: GitHubRepo) => {
        ...
        try {
            ...
            const rootResponse = await fetch(
                `/api/github/repos/${owner}/${repoName}`,
            )
            if (!rootResponse.ok) {
            ...
            if (usageLimits) {
                const remainingImports =
                    usageLimits.limit - (usageLimits.current_usage + 1)
                setUsageLimits({
                    ...usageLimits,
        ...
        for (const item of contents) {
            if (item.type === 'file') {
                ...
                try {
                    ...
                    console.warn(
                        `Failed to fetch directory ${item.path}:`,
                        error,
                    )
                }
            }
                ...
                <CardDescription>
                    ...
                    {usageLimits && (
                        <span className="block mt-2">
                            <Badge
                                variant={
                                    usageLimits.can_import
                                        ? 'secondary'
                                        : 'destructive'
                                }
                                className="mr-2"
                            >
                                {usageLimits.current_usage} /{' '}
                                {usageLimits.is_unlimited
                                    ? '∞'
                                    : usageLimits.limit}{' '}
                                imports used
                            </Badge>
